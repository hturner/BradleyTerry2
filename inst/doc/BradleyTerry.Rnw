...
@
<<citeModel>>=
citeModel <- BTm(cbind(won, lost) ~ -1 + Diff(player1, player2))
citeModel
@ %def
...
@
<<citeModelupdate>>=
update(citeModel, . ~ . - Diff(player1, player2)
       + Diff(player1, player2, refcat = "JASA"))
detach(results)
@ %def
...
citeModel2 <- BTm(cbind(won, lost) ~ -1 +
                  Diff(player1, player2, ~origin, data = journals),
                  data = results)
citeModel2
@ %def
...
@
<<baseballModel>>=
baseballModel1 <- BTm(cbind(home.wins, away.wins) ~
                      -1 + Diff(home.team, away.team), data = baseball)
...
@
<<baseballModelupdate>>=
baseballModel2 <- update(baseballModel1, . ~ . + 1)
