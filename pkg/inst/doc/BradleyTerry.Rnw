...
\\
\url{http://go.warwick.ac.uk/dfirth/software/bradleyterry}}
...
The \BT\ package supersedes the earlier \Rpackage{BradleyTerry} package
...
journal. The \Rfunarg{id} argument specifies that \Rcode{"journal"} is the
...
The \Rfunarg{player1} and \Rfunarg{player2} arguments are either factors
...
\Rcode{at.home} variable seen in Section \ref{sec:order}. The id factors
should be named as specified by the \Rfunarg{id} argument if given in a data
