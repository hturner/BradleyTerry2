...
Bradley-Terry model, with a separate ability parameter estimated for each
player. Section \ref{sec:covariates} considers variations of the standard
...
residuals. Section \ref{sec:bias} introduces biad-reduced estimation, whilst
Section \ref{sec:model} notes the function available to aid model
search. Section \ref{sec:data} explains in more detail how set up data for use
with the \code{BTm} package, Section \ref{sec:functions} lists the functions
provided by the package and finally Section \ref{sec:ties} comments on the
treatment of ties.
...
information on the ability parameters, since these are relative rather than
absolute quantities.
...
analysis---although this affects the significance of the morphological
...
\code{flatlizards} data under the model based on the principal compenents
of the spectral reflectance from the throat are given by
@
<<BTabilities2>>=
head(BTabilities(lizModel2))
@ %def
...
head(cbind(flatlizards$contests, res.pearson))
@ %def
...
@
<<residualWLS>>=
lm(res ~ throat.PC1, weights = attr(res, "weights"), data = flatlizards$predictors)
lm(res ~ head.length, weights = attr(res, "weights"), data = flatlizards$predictors)
@ %def %$
...
Thus for \code{baseballModel2}, which was specified by the following call:
@ 
<<baseballModel2_call>>=
 baseballModel2$call
@ %def 
...
out. Such variables can be provided in a single data frame along with the
...
comparison. For these variables, it is more economical to store the data by
...
described in the last section. The best way to provide these data to \code{BTm}
is a list of data frames, one for each set of variables, e.g.
@ 
<<str_CEMS>>=
str(CEMS)
@ %def 
...
an second player respectively. The rows of data frames in the list should
...
As an example, consider the \code{cemspc} data from the \pkg{prefmod} package,
...
student$ENG <- factor(student$ENG, levels = 1:2, labels = c("good", "poor"))
student$SEX <- factor(student$SEX, levels = 1:2, labels = c("female", "male"))
@ %def 
...
the student data to produce contest-level data. This put in a new data frame
...
@ 
<<school_factors>>=
lab <- c("London", "Paris", "Milano", "St. Gallen", "Barcelona", "Stockholm")
